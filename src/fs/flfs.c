...
int
flfs_init(MountEntry *dev){
    /* nothing */
}

static int
diskread(MountEntry *dev, u_long off, char *buf, int len){
    int i;

    return fbread(dev->f, buf, len, off);
}

static int
diskwrite(MountEntry *dev, u_long off, const char *buf, int len, int erasep){
    int i;

    if( dev->flags & SSF_READONLY ){
        fsmsg("write to read-only device (%s)\n", dev->name);
        return -1;
    }

    /* erase device */
    if( erasep && !(dev->flags & SSF_NOERASE) ){
        if( dev->pblksize > dev->fblksize ){
            ...
            */
            ...
            /* read pblk */
            char * tbuf = alloc(dev->pblksize);
            int toff = off & ~(dev->pblksize-1);

            if( fbread(dev->f, tbuf, dev->pblksize, toff) != dev->pblksize ){
                fsmsg("read %s,%d failed\n", dev->name, toff);
                free(tbuf, dev->pblksize);
                return -1;
            }

            /* erase pblk, copy data, write back */
            fioctl(dev->f, IOC_ERASE, (void*)toff);
            memcpy(tbuf + (off - toff), buf, len);

            i = fbwrite(dev->f, tbuf, dev->pblksize, toff);
            free(tbuf, dev->pblksize);

            if( i != dev->pblksize ){
                fsmsg("rewrite %s,%d failed\n", dev->name, toff);
                return -1;
            }
            return i;
            ...
            for(i=0; i< dev->fblksize/dev->pblksize; i++){
                fioctl(dev->f, IOC_ERASE, (void*)( off + i * dev->pblksize ) );
            }
        }
    }


    return fbwrite(dev->f, buf, len, off);
}


int
chunklength(MountEntry *dev, int offset){
    struct FileData *fd;
    u_long l;
...
static int
findfile(MountEntry *dev, const char *name, char *buffer, int buflen){
    struct FSFile *ff;
    struct FSChunk *fc;
    int nblnk=0;
    int offset, tlen, n;
    ...
    while(1){
        if( diskread(dev, offset, buffer, dev->fblksize) <= 0 ){
            fsmsg("read %s @%d failed (line %d)\n", dev->name, offset, __LINE__);
            return -1;
        }
        ...
        if( fc->type == FCT_BLANK || fc->type == FCT_UNUSED ){
            ...
            if( tlen == 0xFFFFFFFF || tlen == 0 )
                offset += chunklength( dev, offset );
            else
                offset += tlen;
        }

        if( offset >= dev->totaldiskspace || nblnk >= ENOUGHALREADYHEURISTIC ){
...
FILE *
flfs_open_r(MountEntry *dev, const char *name){
    ...
    fd->buffer = alloc( dev->fblksize );
    offset = findfile(dev, name, fd->buffer, dev->fblksize);
    if( offset == -1 ){
        /* file not found */
        free(fd->buffer, dev->fblksize);
    ...
    for(ofd=openforwrite; ofd; ofd=ofd->next){
        if( dev == ofd->dev && offset == ofd->filestart ){
            spin_unlock( &filesystemlock );
            free(fd->buffer, dev->fblksize);
            free(fd, sizeof(struct FileData));
    ...
    if( diskread(dev, offset, fd->buffer, dev->fblksize) <= 0 ){
        spin_unlock( &filesystemlock );
        free(fd->buffer, dev->fblksize);
        free(fd, sizeof(struct FileData));
        fsmsg("read %s,%d failed (line %d)\n", dev->name, offset, __LINE__);
...
flfs_close(FILE *f){
    ...
    if( fd->how == FDH_WRITE ){
        ...
        if( dev->flags & SSF_BLKWRITE ){
            ...
            if( diskwrite(dev, fd->cnkstart, fd->buffer, dev->fblksize, NEEDERASE(fd)) <= 0 ){
                fsmsg("write %s,%d failed (line %d)\n", dev->name, fd->cnkstart, __LINE__);
            }

            if( fd->buffer2 ){
                /* XXX neederase is wrong - (not that it matters) */
                if( diskwrite(dev, fd->filestart, fd->buffer2, dev->fblksize, NEEDERASE(fd)) <= 0 ){
                    fsmsg("write %s,%d failed (line %d)\n", dev->name, fd->filestart, __LINE__);
                }
                free(fd->buffer2, dev->fblksize);
            }
            ...
            if( fd->bufpos ){
                ...
                if( diskwrite(dev, a, fd->buffer, dev->fblksize, NEEDERASE(fd)) <= 0 ){
                    fsmsg("write %s,%d failed (line %d)\n", dev->name, a, __LINE__);
                }
                fd->bufpos = 0;
            }
            ...
            if( diskwrite(dev, fd->cnkstart + offsetof(struct FSChunk, cnklen),
            ...
            i = (fd->cnkpos + dev->fblksize - 1) & ~(dev->fblksize-1);
            if( diskwrite(dev, fd->cnkstart + offsetof(struct FSChunk, totlen),
            ...
            if( i != fd->cnktlen ){
                ...
                if( fd->cnkstat != FCT_BLANK && fd->cnkstat != FCT_UNUSED ){
                    memset(fd->buffer, 0xFF, dev->fblksize);
                    if( diskwrite(dev, fd->cnkstart + i, fd->buffer, dev->fblksize, 1) <= 0 ){
                        fsmsg("write %s,%d failed (line %d)\n", dev->name,
            ...
            if( diskwrite(dev, fd->filestart + sizeof(struct FSChunk) + offsetof(struct FSFile, filelen),
...
flfs_read(FILE* f, char* buf, int len){
    ...
    while(len){
        ...
        if( fd->cnkpos >= fd->cnkdlen + sizeof(struct FSChunk) ){
            ...
            if( diskread(dev, fd->cnknext, fd->buffer, dev->fblksize) <= 0 ){
                fsmsg("read %s,%d failed\n", dev->name, fd->cnknext);
                return -1;
            }
            fc = (struct FSChunk *)fd->buffer;
        ...
        if( fd->bufpos >= fd->buflen ){
            ...
            if( diskread(dev, fd->cnkstart + fd->cnkpos, fd->buffer, dev->fblksize) <= 0 ){
                fsmsg("read %s,%d failed\n", dev->name, fd->cnkstart + fd->cnkpos);
                return -1;
            }
            fd->bufpos = 0;
            fd->buflen = dev->fblksize;
...
findavailable(FILE *f, int type){
    ...
    if( fc->type != FCT_BLANK && fc->type != FCT_UNUSED ){
        /* reused space */
        offset = freed;
        diskread(dev, offset, fd->buffer, dev->fblksize);
        tlen = fc->totlen;
    }else{
        /* virgin space */
        tlen = dev->fblksize;
...
FILE *
flfs_open_w(MountEntry *dev, const char *name){
    struct FSFile *ff;
    struct FileData *fd;
    FILE *f;
    int offset = 0;
    ...
    if( findfile(dev, name, fd->buffer, dev->fblksize) != -1 ){
        /* delete, rename, ... */
...
#    ifdef RENAMEFILE_ON_CREAT
        ...
        while(1){
            ...
            if( findfile(dev, nname, fd->buffer, dev->fblksize) == -1 ){
    ...
    while(len){
        ...
        if( fd->dev->flags & SSF_BLKWRITE ){
            ...
            if( fd->bufpos >= fd->buflen ){
                ...
                fc = (struct FSChunk *)buffer;
                fc->cnklen = fd->cnkpos;
                fc->totlen = fd->dev->fblksize;

                /* get next chunk */
                if( findavailable(f, FCT_CONT) == -1 ){
                    fd->dev->lastfreeoffset = 0;
                    if( findavailable(f, FCT_CONT) == -1 ){
                        /* disk full */
                        fsmsg("filesystem %s is full\n", fd->dev->name);
                        return -1;
                ...
                if( off == fd->filestart ){
                    ...
                    diskwrite(fd->dev, off, buffer, fd->dev->fblksize, NEEDERASE(fd));
            ...
            if( fd->bufpos >= fd->buflen ){
                ...
                diskwrite(fd->dev, a, fd->buffer, fd->dev->fblksize, NEEDERASE(fd));
                fd->bufpos = 0;
            }
            ...
            if( fd->cnkpos >= fd->cnktlen ){
                ...
                offset = fd->cnkstart;
                diskwrite(fd->dev, fd->cnkstart + offsetof(struct FSChunk, cnklen),
                          (char*)&fd->cnkdlen, sizeof(fd->cnkdlen), 0);
                diskwrite(fd->dev, fd->cnkstart + offsetof(struct FSChunk, totlen),
                ...
                if( offset != fd->cnkstart ){
                    diskwrite(fd->dev, offset + offsetof(struct FSChunk, next),
...
FILE *
flfs_open(MountEntry *dev, const char *name, const char *how){
    FILE *f;

    if( *how == 'r' ){
        f = flfs_open_r(dev, name);
    }else{
        f = flfs_open_w(dev, name);
    }
...
int
flfs_dir(MountEntry *dev, int how){
    struct FSFile *ff;
    struct FSChunk *fc;
    char *buffer;
    int offset = 0;
    ...
    if( nblnk && (how & LSHOW_EXT) ){
        printf(" %7.7d(%7.7d)\n", 0, dev->totaldiskspace - offset + nblnk * dev->fblksize);
    }
    ...
    printf("      Total = %8.8d bytes\n"
           "      Free  = %8.8d bytes\n\n",
           total, dev->totaldiskspace - total);
    free(buffer, dev->fblksize);
    return 0;
}
...
int
flfs_chmod(MountEntry *dev, const char *name, int attr){
    struct FSFile *ff;
    char *buffer;
    int offset;
    int n;
    ...
    buffer = alloc(dev->fblksize);
    offset = findfile(dev, name, buffer, dev->fblksize);
    ff = (struct FSFile  *)(buffer + sizeof(struct FSChunk));
    if( offset == -1 ){
        free(buffer, dev->fblksize);
        return -1;
    ...
    diskread(dev, offset, buffer, dev->fblksize);
    ff->attr = attr;
    diskwrite(dev, offset, buffer, dev->fblksize, 1);
    fflush( dev->f );
    free(buffer, dev->fblksize);
    return 0;
}
...
int
flfs_format(MountEntry *dev, char *devname){
    struct FSChunk *fc;
    char *buffer;
    int i;
    ...
    /* allocate space for boot blocks if called as dev:<bootsize> */
    devname += strlen(dev->name);

    if( *devname ){
        i = atoi( devname );
        fc->type = FCT_BIOSEXT;
        i = (i + dev->fblksize - 1) & ~(dev->fblksize - 1);
        fc->totlen = i;
        fc->cnklen = 0;
        fc->next = 0xFFFFFFFF;
        printf("allocating %d for boot block\n", i);
        diskwrite(dev, 0, buffer, dev->fblksize, 1);
    }

    printf("formatting...\n");
    /* skip over any existing boot blocks */
    diskread(dev, 0, buffer, dev->fblksize);
    ...
    memset(buffer, 0xFF, dev->fblksize);
    printf("space=%d blk=%d\n", dev->totaldiskspace, dev->fblksize);
    printf("   0%%");
    for( ; i<dev->totaldiskspace; i+=dev->fblksize){
        diskwrite(dev, i, buffer, dev->fblksize, 1);
        printf("\b\b\b\b%3.3d%%", i * 100 / dev->totaldiskspace);
    }
    printf("\b\b\b\b100%%\n");
    dev->lastfreeoffset = 0;
    fflush( dev->f );
...
int
flfs_deletefile(MountEntry *dev, const char *name){
    struct FSChunk *fc;
    struct FSFile *ff;
    char *buffer;
    int offset;
    ...
    while(1){
        ...
        if( dev->flags & SSF_BLKWRITE ){
            ...
            i = FCT_DELETED;
            diskwrite(dev, offset + offsetof(struct FSChunk, type),
                      (char*)&i, sizeof(i), 0);
        }
        offset = fc->next;
...
int
flfs_renamefile(MountEntry *dev, const char *oname, const char *nname){
    struct FSFile *ff;
    char *buffer;
    int offset;
    int n;
    ...
    buffer = alloc(dev->fblksize);
    if( findfile(dev, nname, buffer, dev->fblksize) != -1 )
        deletefile(nname);

    offset = findfile(dev, oname, buffer, dev->fblksize);
    ff = (struct FSFile *)(buffer + sizeof(struct FSChunk));
    if( offset == -1 ){
        free(buffer, dev->fblksize);
        return -1;
...
int
flfs_glob(MountEntry *dev, const char *pattern, int (*fnc)(const char *, void*), void *args){
    struct FSFile *ff;
    struct FSChunk *fc;
    char *buffer;
    int nblnk=0;
...
int
flfs_ops(int what, MountEntry *dev, ...){
    va_list ap;
    char *name;
    FILE *f;
    int n;
    ...
    case FSOP_RENAME:
        name = va_arg(ap, char*);
        return flfs_renamefile(dev, name, va_arg(ap, char*));

    case FSOP_DELETE:
        return flfs_deletefile(dev, va_arg(ap, char*));

    case FSOP_DIR:
        return flfs_dir(dev, va_arg(ap, int));

    case FSOP_CHMOD:
        name = va_arg(ap, char*);
        return flfs_chmod(dev, name, va_arg(ap, int));
    ...
    case FSOP_FORMAT:
        return flfs_format(dev, va_arg(ap, char*));

    case FSOP_GLOB: {
        char *name = va_arg(ap, char*);
        int (*fnc)(const char *, void*) = va_arg(ap, void*); /* XXX gcc bug */
        return flfs_glob(dev, name, fnc, va_arg(ap, void*) );
    }
    default:
        return -1;
    }
